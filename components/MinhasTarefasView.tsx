                            ...
                            return (
                                <KanbanColumn 
                                    ...
                                    onDrop={(itemId: string) => {
                                        ...
                                        if (tarefaMovida) {
                                            startTransition(() => {
                                                moverTarefaDeColuna(itemId, coluna.id, tarefaMovida.projeto_id)
                                            })
                                        }
                                    }}
                                    onCheck={handleCheck}
...
function KanbanColumn({ titulo, count, tarefas, onDrop, onCheck, onOpen, isPending, isWarning, tipo, dropId }: any) {
    ...
    return (
        <div 
            ...
            className={`w-80 flex-shrink-0 flex flex-col bg-gray-100 rounded-xl border border-gray-200 max-h-full transition-colors ${isOver ? 'bg-indigo-50 border-indigo-300' : ''} ${isWarning ? 'bg-red-50 border-red-100' : ''}`}
        >
...
function DraggableKanbanCard({ tarefa, onCheck, onOpen, isPending }: any) {
    ...
    return (
        <div 
            ...
            className={`bg-white p-3 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-all cursor-default group relative 
        ...
        >
            ...
            <div className="pl-2">
                <div className="flex justify-between items-start mb-1">
                    <span className={`text-[10px] border px-1.5 rounded uppercase font-semibold ${tarefa.concluida ? 'bg-green-50 border-green-100 text-green-600' : 'bg-gray-50 border-gray-100 text-gray-500'}`}>
                        {tarefa.concluida ? 'Concluída' : (tarefa.coluna?.nome || 'A Fazer')}
                    </span>
                    
                    <input 
                        type="checkbox" 
                        checked={tarefa.concluida}
                        onChange={() => onCheck(tarefa.id, tarefa.concluida, tarefa.projeto_id)}
                        onClick={(e) => e.stopPropagation()} 
                        className="w-4 h-4 rounded border-gray-300 text-rose-500 focus:ring-rose-500 cursor-pointer"
                    />
                </div>

                <h4 className={`font-semibold text-gray-800 text-sm mb-2 leading-tight ${tarefa.concluida ? 'line-through text-gray-400' : ''}`}>
                    {tarefa.titulo}
                </h4>
                
                <div className="flex justify-between items-center text-xs text-gray-500 border-t pt-2 border-gray-50">
                    <span className={tarefa.dt_vencimento ? "text-gray-600" : "text-gray-300"}>
                        📅 {formatarData(tarefa.dt_vencimento) || 'S/ Data'}
                    </span>
                    
                    {tarefa.usuario && (
                        <div className="w-5 h-5 rounded-full bg-indigo-100 flex items-center justify-center text-[9px] font-bold text-indigo-700 border border-indigo-200" title={tarefa.usuario.nome}>
