...
import { useState, useTransition, useEffect, ClipboardEvent, KeyboardEvent } from 'react'
...
export default function ModalTarefa({ tarefa, isOpen, onClose, usuarios, projetos, usuarioLogadoId }: Props) {
    ...
    const handleEnviarComentario = async () => {
        ...
        try {
            let imagemFinalUrl = null

            // 1. Upload da Imagem (se houver)
            if (printImagem) {
                const res = await startUpload([printImagem])
                if (res && res[0]) {
                    imagemFinalUrl = res[0].url
                }
            }

            // 2. ID Temporário (UI Otimista)
            const tempId = Math.random().toString()
            const tempComentario = {
                id: tempId,
                texto: novoComentario,
                imagemUrl: previewUrl, // Mostra o preview local instantaneamente
                dt_insert: new Date(),
                usuario: { nome: 'Eu' },
            ...
            <div className="relative bg-surface w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border border-border animate-in zoom-in-95 duration-200">
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar-thin space-y-6 flex-1">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-surface-highlight/10 p-4 rounded-lg border border-border">
                        {/* ... Seus campos de metadados (Etapa, Recorrência, Vencimento, Prioridade...) ... */}
                        {/* (Mantidos idênticos ao seu código original para economizar espaço, se precisar eu repito) */}
                        <div className="col-span-2 sm:col-span-1">
                            ...
                            {modoEdicao ? (
                                <select value={colunaId} onChange={e => setColunaId(e.target.value)} className="w-full bg-surface border border-border rounded px-2 py-1 text-sm text-foreground">
                                    ...
                                    {colunasDisponiveis.map((col: any) => <option key={col.id} value={col.id}>{col.nome}</option>)}
                                </select>
                            ) : (
                                <div className="flex items-center gap-1"><span className="text-sm font-medium text-foreground">{colunasDisponiveis.find((c: any) => c.id === colunaId)?.nome || 'Não Classificado'}</span></div>
                            )}
                        </div>
                        <div className="col-span-2 sm:col-span-1">
                            <label className="block text-[10px] font-bold text-text-muted uppercase mb-1">Recorrência</label>
                            {modoEdicao ? (
                                <select value={recorrencia} onChange={e => setRecorrencia(e.target.value)} className="w-full bg-surface border border-border rounded px-2 py-1 text-sm text-foreground">
